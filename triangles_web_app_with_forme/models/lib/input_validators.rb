...
module InputValidators
  def self.check_side(raw_side)
    side = raw_side || ''
    errors = []
    return errors.concat(['Пустая строка недопустима']) if side == ''

    if side.to_f <= 0
      errors.concat(['Сторона треугольника должна быть положительным числом'])
    end
    {
      side: side.to_f,
      errors: errors
  ...
  def self.check_sides_length(raw_side1, raw_side2, raw_side3)
    ...
    if side1[:errors].empty? && side2[:errors].empty? && side3[:errors].empty?
      ...
      if side1 + side2 > side3 &&
        ...
        errors.concat(['Не выполняется главное неравенство треугольников'])
      end
    else
  ...
  def self.check_min_and_max(raw_min, raw_max)
    ...
    errors = []
    if min == '' && max != ''
      errors.concat(['Не указана минимальная площадь'])
    elsif min != '' && max == ''
      errors.concat(['Не указана максимальная площадь'])
    else
      if min.to_f.negative? || max.to_f.negative?
        errors.concat(['Площадь не может быть отрицательной'])
      end
      if min.to_f > max.to_f
        errors.concat(['Минимальная площадь больше максимальной'])
      end
    end
    {
      errors: errors,
